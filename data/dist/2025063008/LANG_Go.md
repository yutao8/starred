...
- [fatedier/frp](https://github.com/fatedier/frp) ★95589 [`expose`] [`firewall`] [`frp`] [`go`] [`http-proxy`] [`nat`] [`p2p`] [`proxy`] [`reverse-proxy`] [`tunnel`]  
  frp是一个快速、简单且易用的反向代理工具，用于内网穿透。它允许你通过公网访问局域网中的服务。  项目使用Go语言开发，并基于TCP协议实现，提供多种配置选项，支持多种协议转发，例如HTTP、SSH、MySQL等。  frp的核心功能是建立一个安全的连接，将内网服务暴露给公网用户，并提供灵活的路由规则。
- [syncthing/syncthing](https://github.com/syncthing/syncthing) ★73136 [`go`] [`p2p`] [`peer-to-peer`] [`synchronization`]  
...
- [rclone/rclone](https://github.com/rclone/rclone) ★51251 [`azure-blob`] [`azure-blob-storage`] [`azure-files`] [`backblaze-b2`] [`cloud-storage`] [`dropbox`] [`encryption`] [`ftp`] [`fuse-filesystem`] [`go`] [`golang`] [`google-cloud-storage`] [`google-drive`] [`onedrive`] [`openstack-swift`] [`rclone`] [`s3`] [`sftp`] [`sync`] [`webdav`]  
...
- [gogs/gogs](https://github.com/gogs/gogs) ★46557 [`docker`] [`git`] [`go`] [`gogs`] [`mysql`] [`postgresql`] [`raspberry-pi`] [`scip-enabled`] [`self-hosted`] [`sqlite3`]  
  Gogs是一个自托管的Git服务，类似于GitHub和GitLab。它使用Go语言开发，基于Web框架Gin，提供用户管理、代码仓库、分支管理、提交记录等Git功能。Gogs允许用户在本地服务器上搭建Git服务，并控制数据安全和访问权限。  项目包含完整的Web界面，支持多种存储引擎，以及丰富的插件扩展机制。
- [v2ray/v2ray-core](https://github.com/v2ray/v2ray-core) ★46146 [`golang`] [`http-proxy`] [`proxy`] [`shadowsocks`] [`socks`] [`socks5`] [`v2ray`] [`vmess`]  
  V2Ray-core是一个开源的VPN代理服务器，提供快速、安全的网络连接。它支持多种协议，如Shadowsocks、VLESS、Trojan等，并可配置多种加密方式。该项目使用Go语言开发，并采用模块化设计，易于扩展和维护。核心功能包括构建VPN连接、配置服务器和客户端，以及支持自定义路由规则。该项目旨在为用户提供灵活、安全的网络访问方案。
- [usememos/memos](https://github.com/usememos/memos) ★42157 [`docker`] [`go`] [`markdown`] [`memo`] [`microblog`] [`note-taking`] [`notecard`] [`react`] [`self-hosted`] [`social-network`] [`sqlite`]  
...
- [ehang-io/nps](https://github.com/ehang-io/nps) ★32976 [`dns`] [`firewall`] [`go`] [`gzip`] [`http`] [`https`] [`nat`] [`nps`] [`snnapy`] [`socks`] [`socks5`] [`ssh`] [`tcp`] [`tunnel`] [`udp`]  
  这是一个用于收集和分析客户净推荐值 (NPS) 数据的工具。它使用 Python 和 Flask 框架构建，提供用户友好的界面，方便管理 NPS 调查，收集反馈，并生成报告。核心功能包括调查设计、数据收集、结果分析和可视化。该工具支持自定义调查问题和报告模板，并能导出数据到 Excel 或 CSV 格式。
...
- [iawia002/lux](https://github.com/iawia002/lux) ★29780 [`bilibili`] [`crawler`] [`download`] [`downloader`] [`go`] [`golang`] [`iqiyi`] [`qq`] [`scraper`] [`tumblr`] [`video`] [`youku`] [`youtube`]  
  该项目提供一个用于快速构建和部署基于 Lux 的轻量级 Web 应用的工具。它使用 Python 和 Flask 框架，允许用户通过简单的 YAML 配置文件定义应用路由和数据模型。  核心功能包括自动生成 RESTful API 和数据库交互，简化 Web 应用开发流程。  项目包含示例，方便用户快速上手和定制。
- [juanfont/headscale](https://github.com/juanfont/headscale) ★29426 [`tailscale`] [`tailscale-control-server`] [`tailscale-server`] [`wireguard`]  
...
- [Wox-launcher/Wox](https://github.com/Wox-launcher/Wox) ★25784 [`alfred`] [`launcher`] [`launchy`] [`linux`] [`macos`] [`raycast`] [`spotlight`] [`windows`] [`wox`]  
  Wox是一个快速启动应用程序、文件、网页和命令的工具。它使用易于自定义的插件系统，允许用户扩展其功能。Wox使用C++编写，并基于Qt框架。核心功能包括搜索引擎、插件管理和自定义快捷键。用户可以轻松添加自己需要的功能，例如搜索本地文件、执行命令行或访问网络资源。
- [inconshreveable/ngrok](https://github.com/inconshreveable/ngrok) ★24354  
  这是一个用于本地网络服务的反向代理工具。它允许将本地服务器暴露到互联网，方便调试和测试。用户无需配置复杂的防火墙或端口转发规则，即可通过公网访问本地服务。该项目主要使用Go语言编写，并提供多种平台的二进制文件。核心功能包括建立隧道、转发流量以及自定义配置。
- [cloudreve/cloudreve](https://github.com/cloudreve/cloudreve) ★23847 [`cloud`] [`cloud-storage`] [`cloudreve`] [`file`] [`file-manager`] [`file-sharing`] [`golang`]  
  Cloudreve是一个开源的私有云存储服务器，用户可以通过Web界面访问和管理文件。它使用Go语言开发，基于Gin框架，并整合了多种存储后端，如本地文件系统、七牛云、阿里云等。项目提供灵活的权限管理和访问控制，支持多种文件操作，例如上传、下载、分享和同步等功能，方便用户在私有网络中安全地存储和共享文件。
...
- [XIU2/CloudflareSpeedTest](https://github.com/XIU2/CloudflareSpeedTest) ★21998 [`cdn`] [`cloudflare`] [`go`] [`golang`] [`speedtest`]  
...
- [ory/hydra](https://github.com/ory/hydra) ★16306 [`authorization`] [`cloud`] [`docker`] [`federation`] [`hacktoberfest`] [`hydra`] [`identity`] [`oauth`] [`oauth-provider`] [`oauth2`] [`oauth2-provider`] [`oauth2-server`] [`oidc`] [`openid`] [`openid-connect`] [`openid-connect-provider`] [`openid-provider`] [`security`] [`server`] [`sso`]  
  这是一个用于构建和部署微服务的开源平台。Hydra 使用 Go 语言开发，提供了一个易于使用的 API 用于定义和管理微服务，包括服务发现、负载均衡和路由。它基于 Kubernetes，能够自动部署和扩展微服务，并支持多种数据库和消息队列。  该平台的目标是简化微服务架构的开发和维护，提高部署效率和可靠性。
...
- [peterq/pan-light](https://github.com/peterq/pan-light) ★12135 [`baiduyun`] [`golang`] [`qml`] [`qt5`]  
  这是一个轻量级的Python库，用于快速构建和部署简单的网络应用。它基于Flask框架，提供简洁的路由定义和模板引擎，并支持多种数据库连接方式。  核心功能包括：创建RESTful API接口、处理用户请求、模板渲染和数据库交互。  项目使用Python 3和Flask作为主要技术，并整合了SQLAlchemy进行数据库操作。  主要目标是提供一个易于上手、高效的开发工具包，帮助快速搭建小型Web应用。
...
- [OpenListTeam/OpenList](https://github.com/OpenListTeam/OpenList) ★10045 [`alist`] [`aliyunpan`] [`baidupan`] [`openlist`]  
...
- [gokcehan/lf](https://github.com/gokcehan/lf) ★8351 [`file-manager`] [`terminal`]  
  该项目是一个基于Go语言的轻量级文件系统，旨在提供高效的文件存储和检索能力。它使用Go语言的并发特性，并结合了内存映射文件技术，实现快速的文件访问。项目核心功能包括创建、读取、写入和删除文件，以及支持目录操作。代码结构清晰，易于理解和扩展。
- [mindoc-org/mindoc](https://github.com/mindoc-org/mindoc) ★7652 [`beego`] [`document`] [`go`] [`golang`] [`mindoc`] [`smartwiki`]  
  Mindoc 是一个用于生成 Markdown 文档的工具，它能够从各种类型的文档（如 Word、PDF、文本文件）中提取信息，并将其转换为易于维护和使用的 Markdown 格式。它使用 Python 编写，主要依赖于 `PyMuPDF` 库进行 PDF 文件处理，`tika` 库处理其他文档类型。项目提供了命令行工具和 Python API，方便用户进行批量文档转换。  该工具特别适用于需要将现有文档转换为 Markdown 格式以进行后续编辑和维护的场景。
...
- [jpillora/cloud-torrent](https://github.com/jpillora/cloud-torrent) ★6009 [`cloud`] [`golang`] [`self-hosted`] [`torrent`] [`torrent-client`]  
  这是一个基于Go语言的云端种子下载器。它利用`gin`框架构建API接口，允许用户上传种子文件并下载其内容。  项目使用`BitTorrent`协议进行种子下载，并支持多线程下载，提高下载速度。  核心功能包括种子上传、下载任务管理、进度监控和文件存储。  该项目旨在提供一个简单易用的云端种子下载平台，方便用户管理和下载种子文件。
...
- [git-jiadong/wechatDataBackup](https://github.com/git-jiadong/wechatDataBackup) ★5006 [`wechat`] [`wechat-app`] [`wechat-bot`]  
  这是一个用于微信聊天记录备份的工具。它使用Python编写，基于微信网页版接口，支持批量下载微信好友、群聊的聊天记录和文件。项目包含了数据解析和存储模块，方便用户管理备份数据。  主要使用requests库进行网页请求，并利用正则表达式提取关键信息。  该工具旨在提供一种便捷的微信聊天记录备份方式，避免数据丢失。
...
- [fanpei91/torsniff](https://github.com/fanpei91/torsniff) ★4010 [`bep0003`] [`bep0005`] [`bep0009`] [`bep0010`] [`bittorrent-protocol`] [`dht`] [`golang`] [`metadata`]  
  这是一个用于分析和可视化Tor网络流量的工具。它使用Python和Scapy库，通过捕获和解码Tor流量包来识别Tor节点和路径。  核心功能包括流量包捕获、节点识别、路径追踪以及流量统计。  项目提供了一个直观的图形界面，方便用户查看分析结果，并支持自定义过滤器。  该工具主要面向网络安全研究人员和渗透测试人员，帮助他们深入了解Tor网络结构和流量行为。
...
- [qjfoidnh/BaiduPCS-Go](https://github.com/qjfoidnh/BaiduPCS-Go) ★3670 [`baidupcs`]  
  BaiduPCS-Go 是一个基于 Go 语言的百度网盘客户端。它提供便捷的本地文件管理和同步功能，允许用户通过命令行或 GUI 界面访问和操作百度网盘文件。该项目使用 Go 语言开发，并利用了 `github.com/spf13/viper` 等库进行配置管理，以及 `github.com/gin-gonic/gin` 进行 Web API 接口。  它支持断点续传、文件上传和下载等核心功能，并提供多种自定义配置选项。
...
- [fagongzi/manba](https://github.com/fagongzi/manba) ★3309 [`api`] [`api-gateway`] [`backend-apis`] [`circuit-breaker`] [`etcd`] [`gateway`] [`go`] [`golang`] [`microservice`] [`proxy`] [`traffic-control`]  
  这是一个用于管理和监控漫步机器人（例如：扫地机器人）的项目。它提供了一个Web界面，允许用户查看机器人的当前状态、控制其移动、设置虚拟围栏以及查看历史记录。该项目使用Python和Flask框架开发，并使用MQTT协议与机器人进行通信。关键模块包括机器人状态监控、远程控制、地图管理和日志记录。
...
- [eolinker/goku_lite](https://github.com/eolinker/goku_lite) ★3130 [`api`] [`api-gateway`] [`gateway`] [`goku`] [`microservices`] [`middleware`] [`proxy`]  
  Goku Lite是一个轻量级的API管理工具，用于快速创建、测试和部署API。它使用Go语言开发，基于Gin框架，提供RESTful API接口定义和管理功能。用户可以通过简单的YAML文件定义API，并使用内置工具进行测试和调试。Goku Lite的目标是提供一个易于上手、高效的API管理解决方案，适用于小型项目或快速迭代开发。
...
- [0xDkd/auxpi](https://github.com/0xDkd/auxpi) ★2715 [`api`] [`baidu-image`] [`cdnjs`] [`element-ui`] [`go`] [`golang`] [`image-store`] [`mdui`] [`rest-api`] [`vue`] [`weibo-image`]  
  auxpi 是一个用于辅助 Python 项目开发的工具库。它提供了一系列便捷的函数，用于处理文件、目录、日志、配置等任务，从而简化 Python 项目的开发流程。auxpi 使用 Python 编写，并基于标准库和第三方库，如 `pathlib`、`configparser` 等，实现高效的代码复用和模块化。  核心功能包括：文件操作、目录管理、日志记录、配置读取以及常用工具函数。  它旨在提高 Python 项目开发效率，并减少重复代码。
...
- [ssstk/cngrok](https://github.com/ssstk/cngrok) ★363  
  这是一个基于Go语言编写的轻量级反向代理工具，用于本地网络服务映射到公网。它使用Cngrok作为底层库，提供简单的命令行界面，方便用户快速配置和管理本地服务的访问。  项目使用Go语言和Cngrok库构建，支持自定义端口映射和域名绑定，并提供基本的日志记录功能，方便调试和监控。  该工具主要用于开发和测试阶段，将本地服务暴露到公网，方便团队协作和远程访问。
- [upyun/upx](https://github.com/upyun/upx) ★199 [`cli`] [`upyun`]  
  UPX是一个用于Linux、Windows和macOS的通用文件压缩工具。它使用多种技术来压缩可执行文件、库和其它类型的文件。主要使用C语言开发，并通过精心设计的算法和数据结构，在不影响程序运行速度的前提下，最大程度地减小文件大小。该项目提供命令行工具和库接口，方便用户集成到其他应用中。
