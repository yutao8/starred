...
- [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) ★46552 [`antigravity`] [`claude-code`] [`cluade`] [`codex`] [`gemini`] [`openai`]  
  CLIProxyAPI 是一个用于简化命令行工具与 API 服务之间交互的代理接口，主要用于将 CLI 请求转发为 HTTP 调用，支持鉴权、参数映射和响应格式化，适合集成第三方服务或构建自动化工作流。
- [QuantumNous/new-api](https://github.com/QuantumNous/new-api) ★44676 [`ai-gateway`] [`claude`] [`deepseek`] [`gemini`] [`newapi`] [`openai`] [`rerank`]  
  该项目主要用于构建新一代API服务，聚焦于提升系统间通信效率与数据处理能力，通常适用于需要高并发、低延迟场景的分布式系统集成，帮助开发者简化后端服务对接与业务逻辑封装。
- [juanfont/headscale](https://github.com/juanfont/headscale) ★42630 [`tailscale`] [`tailscale-control-server`] [`tailscale-server`] [`wireguard`]  
...
- [schollz/croc](https://github.com/schollz/croc) ★39509 [`data-transfer`] [`file-sharing`] [`golang`] [`pake`] [`peer-to-peer`] [`tcp`] [`transfer`]  
...
- [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api) ★36314 [`2api`] [`antigravity2api`] [`cc2api`] [`claude`] [`claude-code`] [`codex`] [`crs`] [`crs2`] [`gemini`]  
...
- [tailscale/tailscale](https://github.com/tailscale/tailscale) ★34947 [`2fa`] [`oauth`] [`sso`] [`tailscale`] [`vpn`] [`wireguard`]  
...
- [binwiederhier/ntfy](https://github.com/binwiederhier/ntfy) ★33299 [`curl`] [`notifications`] [`ntfy`] [`ntfysh`] [`pubsub`] [`push-notifications`] [`rest-api`]  
...
- [XIU2/CloudflareSpeedTest](https://github.com/XIU2/CloudflareSpeedTest) ★28354 [`cdn`] [`cloudflare`] [`go`] [`golang`] [`speedtest`]  
...
- [Wox-launcher/Wox](https://github.com/Wox-launcher/Wox) ★27195 [`alfred`] [`launcher`] [`launchy`] [`productivity`] [`productivity-tools`] [`raycast`] [`spotlight`] [`wox`] [`wox-launcher`]  
...
- [GopeedLab/gopeed](https://github.com/GopeedLab/gopeed) ★25704 [`android`] [`bittorrent`] [`cross-platform`] [`debian`] [`downloader`] [`flutter`] [`golang`] [`http`] [`https`] [`ios`] [`macos`] [`magnet`] [`torrent`] [`ubuntu`] [`windows`]  
...
- [OpenListTeam/OpenList](https://github.com/OpenListTeam/OpenList) ★23953 [`alist`] [`aliyunpan`] [`baidupan`] [`openlist`]  
  一个开源的列表协作工具，支持多人实时编辑和数据同步，可用于任务管理、资源整理等场景，提供简洁的用户界面和灵活的数据组织方式，便于团队高效协作与信息共享。
- [navidrome/navidrome](https://github.com/navidrome/navidrome) ★22761 [`airsonic`] [`madsonic`] [`media-server`] [`music`] [`music-server`] [`navidrome`] [`opensubsonic`] [`raspberry-pi`] [`self-hosted`] [`streamer`] [`streaming-api`] [`streaming-audio`] [`subsonic`] [`subsonic-server`]  
...
- [chaitin/SafeLine](https://github.com/chaitin/SafeLine) ★22315 [`api-gateway`] [`application-security`] [`appsec`] [`blueteam`] [`bruteforce`] [`captcha`] [`cve`] [`cybersecurity`] [`firewall`] [`hackers`] [`http-flood`] [`security`] [`self-hosted`] [`sql-injection`] [`vulnerability`] [`waf`] [`web-application-firewall`] [`web-security`] [`websecurity`] [`xss`]  
...
- [m1k1o/neko](https://github.com/m1k1o/neko) ★21886 [`docker`] [`golang`] [`media-streaming`] [`remote-control`] [`remote-desktop`] [`self-hosted`] [`virtual-browser`] [`vue`] [`webrtc`]  
  neko 是一个基于浏览器的虚拟桌面，可在 Docker 中运行，提供完整操作系统体验，支持通过 WebRTC 实现低延迟远程访问，适用于远程办公、临时开发环境或快速部署隔离桌面会话。
- [lionsoul2014/ip2region](https://github.com/lionsoul2014/ip2region) ★19394 [`ip-address`] [`ip-address-database`] [`ip-address-location`] [`ip-location`] [`ip-lookup`] [`ipv4-address`] [`ipv4-lookup`] [`ipv4-support`] [`ipv6-address`] [`ipv6-lookup`] [`ipv6-support`] [`xdb-engine`] [`xdb-ipv4`] [`xdb-ipv6`]  
...
- [putyy/res-downloader](https://github.com/putyy/res-downloader) ★19007 [`douyin`] [`kuaishou`] [`res-downloader`] [`wechat`] [`wechat-video`] [`xiaohongshu`]  
...
- [slackhq/nebula](https://github.com/slackhq/nebula) ★17594  
...
- [jeessy2/ddns-go](https://github.com/jeessy2/ddns-go) ★17181 [`alidns`] [`baiduyun`] [`cloudflare`] [`ddns`] [`ddns-go`] [`dnspod`] [`porkbun`]  
...
- [gotify/server](https://github.com/gotify/server) ★15675 [`api`] [`cloud`] [`free-software`] [`golang`] [`gotify`] [`hosting`] [`javascript`] [`notifications`] [`privacy`] [`react`] [`self-hosted`] [`self-hosting`] [`selfhosted`]  
...
- [cloudflare/cloudflared](https://github.com/cloudflare/cloudflared) ★15134 [`cloudflare`] [`cloudflare-tunnel`] [`reverse-proxy`] [`zero-trust-network-access`]  
...
- [coredns/coredns](https://github.com/coredns/coredns) ★14230 [`cncf`] [`coredns`] [`dns-server`] [`go`] [`plugin`] [`service-discovery`]  
...
- [certimate-go/certimate](https://github.com/certimate-go/certimate) ★9018 [`acme`] [`acme-client`] [`automation`] [`certbot`] [`certificate`] [`certificate-management`] [`certificate-manager`] [`certificates`] [`devops`] [`google-trust-services`] [`https`] [`https-certificate`] [`lego`] [`letsencrypt`] [`self-hosted`] [`ssl`] [`ssl-cert`] [`ssl-certificate`] [`ssl-certificates`] [`zerossl`]  
...
- [ViRb3/wgcf](https://github.com/ViRb3/wgcf) ★8578 [`client`] [`cloudflare`] [`plus`] [`security`] [`vpn`] [`warp`] [`wireguard`]  
...
- [gdy666/lucky](https://github.com/gdy666/lucky) ★8022 [`acme`] [`aliyunpan`] [`cron`] [`ddns`] [`dynadot`] [`filebrowser`] [`ftp`] [`ipv6`] [`socat`] [`stun`] [`vercel`] [`volcengine`] [`webdav`] [`wol`]  
...
- [koho/frpmgr](https://github.com/koho/frpmgr) ★2033 [`frp`] [`frpc`] [`go`] [`gui`] [`proxy`] [`reverse-proxy`] [`windows`]  
