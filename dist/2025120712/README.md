...
- **编程语言种类：** 54
- **仓库总数：** 1839
- **最后更新时间：** 2025-12-07 12:00:19

[**→ 查看所有仓库**](dist/2025120712/ALL.md)

## 最近收藏（10 个）

- [dnSpy/dnSpy](https://github.com/dnSpy/dnSpy) ★28643  
...
- [mpv-player/mpv](https://github.com/mpv-player/mpv) ★32881 [`audio`] [`c`] [`ffmpeg`] [`mplayer`] [`mpv`] [`multimedia`] [`video`]  
...
- [massgravel/Microsoft-Activation-Scripts](https://github.com/massgravel/Microsoft-Activation-Scripts) ★158605 [`activator`] [`hwid`] [`kms`] [`kms38`] [`massgrave`] [`massgravel`] [`microsoft`] [`microsoft365`] [`office`] [`office365`] [`ohook`] [`powershell`] [`tsforge`] [`windows`] [`windows-10`] [`windows-11`]  
...
- [Atlas-OS/Atlas](https://github.com/Atlas-OS/Atlas) ★18807 [`ame-wizard`] [`atlas`] [`atlasos`] [`debloat`] [`fps`] [`gaming`] [`latency`] [`open-source`] [`opensource`] [`performance`] [`privacy`] [`security`] [`tweaks`] [`windows`]  
...
- [Genymobile/scrcpy](https://github.com/Genymobile/scrcpy) ★132084 [`android`] [`c`] [`ffmpeg`] [`libav`] [`mirroring`] [`recording`] [`screen`] [`sdl2`]  
...
- [ventoy/Ventoy](https://github.com/ventoy/Ventoy) ★72789 [`auto-install`] [`bootable-usb`] [`bsd`] [`chromeos`] [`iso-files`] [`legacy`] [`linux`] [`multiboot`] [`persistence`] [`secure-boot`] [`uefi`] [`unattended`] [`unix`] [`usb`] [`windows`]  
...
- [jqlang/jq](https://github.com/jqlang/jq) ★33146 [`jq`]  
...
技术实现方面，mpv主要用C语言编写，利用FFmpeg实现解码、libass负责ASS/SSA字幕渲染，SDL2或Wayland为默认图形输出。项目将渲染流程拆分为解码线程、音频线程、视频渲染线程，并通过libmpv API提供给开发者用于嵌入或调用。Lua脚本语言被广泛用于插件和UI定制，脚本可访问视频信息、播放状态并直接操作播放器行为。整体架构高度模块化，支持Linux、Windows、macOS、BSD等多平台编译，保证跨平台一致性体验。
- [coolsnowwolf/lede](https://github.com/coolsnowwolf/lede) ★31154 [`lede`] [`lua`] [`openwrt-feed`] [`openwrt-package`] [`openwrt-zh-cn`]  
...
- [valinet/ExplorerPatcher](https://github.com/valinet/ExplorerPatcher) ★30611  
...
- [openssl/openssl](https://github.com/openssl/openssl) ★29166 [`cryptography`] [`decryption`] [`encryption`] [`openssl`] [`ssl`] [`tls`]  
...
- [kingToolbox/WindTerm](https://github.com/kingToolbox/WindTerm) ★28978 [`bash`] [`client`] [`cmd`] [`linux`] [`mac`] [`powershell`] [`serial`] [`sftp`] [`shell`] [`ssh`] [`telnet`] [`terminal`] [`tmux`] [`windows`] [`wsl`] [`x11`] [`xmodem`] [`xterm`] [`ymodem`] [`zmodem`]  
...
- [ggreer/the_silver_searcher](https://github.com/ggreer/the_silver_searcher) ★27144 [`ag`] [`c`] [`command-line-tool`] [`pcre`] [`search-in-text`] [`silver-searcher`]  
...
[查看更多 C 仓库 →](dist/2025120712/LANG_C.md)

## C#

- [microsoft/PowerToys](https://github.com/microsoft/PowerToys) ★126275 [`color-picker`] [`command-palette`] [`desktop`] [`fancyzones`] [`keyboard-manager`] [`microsoft-powertoys`] [`powerrename`] [`powertoys`] [`windows`] [`windows-10`] [`windows-11`]  
...
- [2dust/v2rayN](https://github.com/2dust/v2rayN) ★91686 [`proxy`] [`shadowsocks`] [`socks5`] [`trojan`] [`v2fly`] [`v2ray`] [`vless`] [`vmess`] [`windows`] [`xray`] [`xtls`]  
...
- [shadowsocks/shadowsocks-windows](https://github.com/shadowsocks/shadowsocks-windows) ★59158 [`c-sharp`] [`proxy`] [`shadowsocks`]  
...
- [PowerShell/PowerShell](https://github.com/PowerShell/PowerShell) ★50933 [`command-line`] [`hacktoberfest`] [`linux`] [`macos`] [`netcore`] [`powershell`] [`shell`] [`windows`]  
  这是一个用于PowerShell的项目，具体功能不明确。  需要更多信息才能生成合适的项目描述。 请提供GitHub项目链接。
- [huiyadanli/RevokeMsgPatcher](https://github.com/huiyadanli/RevokeMsgPatcher) ★35828 [`hex-editor`] [`patch`] [`pc`] [`qq`] [`revoke`] [`revokemsg`] [`tim`] [`tool`] [`wechat`] [`windows`]  
...
- [AvaloniaUI/Avalonia](https://github.com/AvaloniaUI/Avalonia) ★29450 [`android`] [`app-framework`] [`avalonia`] [`avaloniaui`] [`browser`] [`c-sharp`] [`cross-platform`] [`csharp`] [`desktop`] [`dotnet`] [`gui`] [`ios`] [`linux`] [`macos`] [`mobile`] [`mvvm`] [`wasm`] [`windows`] [`xamarin`] [`xaml`]  
...
- [dnSpy/dnSpy](https://github.com/dnSpy/dnSpy) ★28643  
  dnSpy是一款开源的 .NET 程序集浏览、反编译、编辑与调试工具，可直接在 Windows 环境下对 .exe、.dll 进行源码查看并修改后重新保存。核心采用 C# 编写，基于 WinForms 实现 UI，使用 ICSharpCode.Decompiler（SharpDevelop）提供高精度的 C#/VB 反编译，引入 Mono.Cecil 进行程序集读取与写入，集成自研的调试引擎实现实时断点、变量监视及堆栈回溯。项目结构划分为 UI 层、反编译层、程序集操作层和调试层，插件机制支持自定义脚本与扩展功能，便于安全分析、逆向工程及快速修补 .NET 应用程序。
- [rocksdanister/lively](https://github.com/rocksdanister/lively) ★17701 [`csharp`] [`desktop`] [`desktop-wallpaper`] [`dynamic-desktop`] [`dynamic-wallpaper`] [`fluent-design`] [`live-wallpaper`] [`screensavers`] [`wallpaper`] [`wallpaper-app`] [`wallpaper-changer`] [`wallpaper-engine`] [`wallpaperengine`] [`wallpapers`] [`widgets`] [`windows-10`] [`windows-11`] [`windows-app`] [`winui`] [`winui3`]  
...
- [bitwarden/server](https://github.com/bitwarden/server) ★17599 [`api`] [`aspnet`] [`aspnetcore`] [`bitwarden`] [`csharp`] [`docker`] [`dotnet`] [`dotnet-core`] [`signalr`] [`sql`] [`sql-server`]  
...
- [netchx/netch](https://github.com/netchx/netch) ★17146 [`fullcone`] [`netch`] [`netfiltersdk`] [`shadowsocks`] [`shadowsocksr`] [`socks`] [`tun2socks`] [`vless`] [`vmess`]  
...
[查看更多 C# 仓库 →](dist/2025120712/LANG_C_.md)

## C++

- [electron/electron](https://github.com/electron/electron) ★119282 [`c-plus-plus`] [`chrome`] [`css`] [`electron`] [`html`] [`javascript`] [`nodejs`] [`v8`] [`works-with-codespaces`]  
...
- [microsoft/terminal](https://github.com/microsoft/terminal) ★101032 [`cmd`] [`command-line`] [`console`] [`contributions-welcome`] [`good-first-issue`] [`hacktoberfest`] [`terminal`] [`windows`] [`windows-console`] [`windows-terminal`] [`wsl`]  
...
- [tesseract-ocr/tesseract](https://github.com/tesseract-ocr/tesseract) ★71265 [`hacktoberfest`] [`lstm`] [`machine-learning`] [`ocr`] [`ocr-engine`] [`tesseract`] [`tesseract-ocr`]  
...
- [aria2/aria2](https://github.com/aria2/aria2) ★39423 [`bittorrent`] [`cpp11`] [`download`] [`ftp`] [`http`] [`metalink`] [`rpc`] [`sftp`]  
...
- [qbittorrent/qBittorrent](https://github.com/qbittorrent/qBittorrent) ★34316 [`bittorrent`] [`bittorrent-client`] [`c-plus-plus`] [`crossplatform`] [`torrent`] [`torrent-client`]  
...
- [LizardByte/Sunshine](https://github.com/LizardByte/Sunshine) ★32089 [`cpp`] [`docker`] [`flathub-pkg`] [`game-stream`] [`game-streaming`] [`game-streaming-server`] [`geforce-experience`] [`homebrew-pkg`] [`maintainer-wanted`] [`moonlight`] [`pacman-pkg`] [`python`] [`remote-desktop`] [`replicator-custom-issues`] [`replicator-release-notifications`] [`rtd`] [`winget-pkg`]  
...
- [c0re100/qBittorrent-Enhanced-Edition](https://github.com/c0re100/qBittorrent-Enhanced-Edition) ★24437 [`bittorrent`] [`bittorrent-client`]  
...
- [google/filament](https://github.com/google/filament) ★19150 [`3d-graphics`] [`android`] [`gltf`] [`gltf-viewer`] [`graphics`] [`metal`] [`opengl`] [`opengl-es`] [`pbr`] [`real-time`] [`vulkan`] [`wasm`] [`webgl`]  
...
[查看更多 C++ 仓库 →](dist/2025120712/LANG_C__.md)
...
- [microsoft/vcpkg](https://github.com/microsoft/vcpkg) ★26232 [`c`] [`cmake`] [`cplusplus`] [`cpp`] [`libraries`] [`package-manager`] [`packages`] [`vcpkg`] [`visual-studio`] [`windows`]  
...
- [FelisCatus/SwitchyOmega](https://github.com/FelisCatus/SwitchyOmega) ★22444  
...
- [zero-peak/ZeroOmega](https://github.com/zero-peak/ZeroOmega) ★6600 [`proxy`] [`switchyomega`] [`zeroomega`]  
...
- [thedevdojo/wave](https://github.com/thedevdojo/wave) ★6371 [`devdojo`] [`hacktoberfest`] [`laravel`] [`php`] [`saas`] [`saas-boilerplate`] [`subscription`] [`tailwindcss`] [`voyager`] [`wave`]  
...
[查看更多 CSS 仓库 →](dist/2025120712/LANG_CSS.md)
...
- [AppFlowy-IO/AppFlowy](https://github.com/AppFlowy-IO/AppFlowy) ★66822 [`blog`] [`confluence-alternative`] [`content-management`] [`content-services`] [`documentation`] [`flutter`] [`note-taking`] [`notion-alternative`] [`project-management`] [`task-management`] [`team-collaboration`] [`wiki`]  
...
- [GopeedLab/gopeed](https://github.com/GopeedLab/gopeed) ★22005 [`android`] [`bittorrent`] [`cross-platform`] [`debian`] [`downloader`] [`flutter`] [`golang`] [`http`] [`https`] [`ios`] [`macos`] [`magnet`] [`torrent`] [`ubuntu`] [`windows`]  
...
- [guozhigq/pilipala](https://github.com/guozhigq/pilipala) ★13003 [`bilibili`] [`dart`] [`flutter`] [`material-ui`]  
...
- [wanghongenpin/proxypin](https://github.com/wanghongenpin/proxypin) ★11137 [`capture-traffic`] [`charles`] [`fiddler`] [`proxy`] [`proxypin`] [`zhuabao`]  
...
- [ComicSparks/pikapika](https://github.com/ComicSparks/pikapika) ★7710 [`acg`] [`android`] [`bika`] [`comic-books`] [`cosplay`] [`cross-platform`] [`e-hentai`] [`flutter`] [`game`] [`go`] [`golang`] [`hentai`] [`ios`] [`picacg`] [`picacomic`] [`r-18`]  
...
- [Predidit/oneAnime](https://github.com/Predidit/oneAnime) ★1834 [`android`] [`anime`] [`anime1`] [`danmaku`] [`flutter`] [`ios`] [`linux-desktop`] [`macos`] [`windows`]  
...
[查看更多 Dart 仓库 →](dist/2025120712/LANG_Dart.md)

## Dockerfile

- [Anduin2017/HowToCook](https://github.com/Anduin2017/HowToCook) ★96211 [`chinese`] [`cookbook`] [`cooking`] [`dishes`] [`recipes`]  
...
- [jaywcjlove/reference](https://github.com/jaywcjlove/reference) ★14754 [`cheatsheet`] [`docker`] [`javascript`] [`npm`] [`npm-package`] [`react`] [`reactjs`] [`references`] [`semver`] [`toml`] [`typescript`] [`vue3`] [`vuejs`]  
...
- [RICwang/docker-wechat](https://github.com/RICwang/docker-wechat) ★881  
...
- [Fechin/reference](https://github.com/Fechin/reference) ★9602 [`awk`] [`bash`] [`chatgpt`] [`cheatsheet`] [`cheatsheets`] [`css`] [`golang`] [`grep`] [`markdown`] [`python`] [`reference`] [`sed`] [`snippets`] [`vim`]  
...
- [Richasy/Bili.Copilot](https://github.com/Richasy/Bili.Copilot) ★4913 [`bilibili`] [`windows-app-sdk`] [`winui3`]  
...
- [fatedier/frp](https://github.com/fatedier/frp) ★101419 [`expose`] [`firewall`] [`frp`] [`go`] [`http-proxy`] [`nat`] [`p2p`] [`proxy`] [`reverse-proxy`] [`tunnel`]  
...
- [syncthing/syncthing](https://github.com/syncthing/syncthing) ★78090 [`go`] [`p2p`] [`peer-to-peer`] [`synchronization`]  
...
- [junegunn/fzf](https://github.com/junegunn/fzf) ★75578 [`bash`] [`cli`] [`fish`] [`fzf`] [`go`] [`neovim`] [`tmux`] [`unix`] [`vim`] [`zsh`]  
...
- [minio/minio](https://github.com/minio/minio) ★59015 [`amazon-s3`] [`cloud`] [`cloudnative`] [`cloudstorage`] [`go`] [`k8s`] [`kubernetes`] [`multi-cloud`] [`multi-cloud-kubernetes`] [`objectstorage`] [`s3`] [`storage`]  
...
- [FiloSottile/mkcert](https://github.com/FiloSottile/mkcert) ★57353 [`certificates`] [`chrome`] [`firefox`] [`https`] [`ios`] [`linux`] [`local-development`] [`localhost`] [`macos`] [`root-ca`] [`tls`] [`windows`]  
...
- [rclone/rclone](https://github.com/rclone/rclone) ★53935 [`azure-blob`] [`azure-blob-storage`] [`azure-files`] [`backblaze-b2`] [`cloud-storage`] [`dropbox`] [`encryption`] [`ftp`] [`fuse-filesystem`] [`go`] [`golang`] [`google-cloud-storage`] [`google-drive`] [`onedrive`] [`openstack-swift`] [`rclone`] [`s3`] [`sftp`] [`sync`] [`webdav`]  
...
- [go-gitea/gitea](https://github.com/go-gitea/gitea) ★52369 [`bitbucket`] [`cicd`] [`devops`] [`docker-registry-v2`] [`git`] [`git-gui`] [`git-server`] [`gitea`] [`github`] [`github-actions`] [`gitlab`] [`go`] [`golang`] [`hacktoberfest`] [`maven-server`] [`npm-registry`] [`vue`]  
...
- [AlistGo/alist](https://github.com/AlistGo/alist) ★48618 [`file-server`] [`gin`] [`golang`] [`onedrive`] [`solidjs`] [`webdav`]  
...
- [gogs/gogs](https://github.com/gogs/gogs) ★47292 [`docker`] [`git`] [`go`] [`gogs`] [`mysql`] [`postgresql`] [`raspberry-pi`] [`scip-enabled`] [`self-hosted`] [`sqlite3`]  
...
- [usememos/memos](https://github.com/usememos/memos) ★46907 [`docker`] [`foss`] [`go`] [`markdown`] [`memo`] [`microblog`] [`note-taking`] [`notecard`] [`react`] [`self-hosted`] [`social-network`] [`sqlite`]  
...
[查看更多 Go 仓库 →](dist/2025120712/LANG_Go.md)

## Haskell

- [jgm/pandoc](https://github.com/jgm/pandoc) ★40668 [`commonmark`] [`converter`] [`document`] [`haskell`] [`markdown`] [`markup`] [`pandoc`] [`presentation`] [`publishing`]  
...
- [ripienaar/free-for-dev](https://github.com/ripienaar/free-for-dev) ★116020 [`awesome-list`] [`free-for-developers`]  
...
- [vernesong/OpenClash](https://github.com/vernesong/OpenClash) ★22972 [`clash`] [`luci`] [`mihomo`] [`openclash`] [`openwrt`] [`shadowsocks`] [`snell`] [`v2ray`] [`vmess`]  
...
- [Lerist/FakeLocation](https://github.com/Lerist/FakeLocation) ★6669  
...
- [CaiJimmy/hugo-theme-stack](https://github.com/CaiJimmy/hugo-theme-stack) ★6109 [`bloggers`] [`css`] [`hacktoberfest`] [`hugo`] [`hugo-theme`] [`theme`]  
...
- [webmin/webmin](https://github.com/webmin/webmin) ★5417  
...
- [vbskycn/iptv](https://github.com/vbskycn/iptv) ★4986  
...
[查看更多 HTML 仓库 →](dist/2025120712/LANG_HTML.md)

## Java

- [dbeaver/dbeaver](https://github.com/dbeaver/dbeaver) ★47640 [`ai`] [`copilot`] [`database`] [`db2`] [`dbeaver`] [`erd`] [`gui`] [`java`] [`jdbc`] [`mysql`] [`nosql`] [`openai`] [`oracle`] [`postgresql`] [`redshift`] [`sql`] [`sqlite`] [`sqlserver`]  
...
- [termux/termux-app](https://github.com/termux/termux-app) ★47498 [`android`] [`hacktoberfest`] [`linux`] [`terminal`] [`termux`]  
...
- [skylot/jadx](https://github.com/skylot/jadx) ★46412 [`android`] [`decompiler`] [`dex`] [`java`]  
...
- [halo-dev/halo](https://github.com/halo-dev/halo) ★37386 [`blog`] [`blog-engine`] [`cms`] [`content-management-system`] [`halo`] [`halocms`] [`website-builder`]  
...
- [DrKLO/Telegram](https://github.com/DrKLO/Telegram) ★28058 [`telegram`]  
...
- [dataease/dataease](https://github.com/dataease/dataease) ★22593 [`apache-doris`] [`business-intelligence`] [`data-analysis`] [`data-visualization`] [`echarts`] [`kettle`] [`superset`] [`tableau`]  
...
- [lionsoul2014/ip2region](https://github.com/lionsoul2014/ip2region) ★18447 [`ip-address`] [`ip-address-database`] [`ip-address-location`] [`ip-location`] [`ip-lookup`] [`ipv4-address`] [`ipv4-lookup`] [`ipv4-support`] [`ipv6-address`] [`ipv6-lookup`] [`ipv6-support`] [`xdb-engine`] [`xdb-ipv4`] [`xdb-ipv6`]  
...
[查看更多 Java 仓库 →](dist/2025120712/LANG_Java.md)

## JavaScript

- [louislam/uptime-kuma](https://github.com/louislam/uptime-kuma) ★79422 [`docker`] [`monitor`] [`monitoring`] [`responsive`] [`self-hosted`] [`selfhosted`] [`single-page-app`] [`socket-io`] [`uptime`] [`uptime-monitoring`] [`webapp`] [`websocket`]  
...
- [marktext/marktext](https://github.com/marktext/marktext) ★52646 [`dark-mode`] [`editor`] [`electron`] [`element-ui`] [`emoji`] [`focus-mode`] [`latex`] [`linux`] [`mac`] [`macos`] [`markdown`] [`marktext`] [`next-generation`] [`source-code`] [`typewriter-mode`] [`vue`] [`windows`]  
...
- [agalwood/Motrix](https://github.com/agalwood/Motrix) ★49956 [`aria2`] [`bittorrent`] [`bt`] [`download`] [`download-manager`] [`electron`] [`linux`] [`mac`] [`macos`] [`magnet`] [`motrix`] [`torrent`] [`windows`]  
...
- [LeCoupa/awesome-cheatsheets](https://github.com/LeCoupa/awesome-cheatsheets) ★44902 [`backend`] [`bash`] [`cheatsheet`] [`cheatsheets`] [`database`] [`django`] [`docker`] [`feathersjs`] [`frontend`] [`javascript`] [`kubernetes`] [`language`] [`nodejs`] [`php`] [`programming-language`] [`redis`] [`sailsjs`] [`vim`] [`vuejs`] [`xcode`]  
...
- [HeyPuter/puter](https://github.com/HeyPuter/puter) ★37651 [`cloud`] [`cloud-os`] [`cloud-storage`] [`desktop`] [`desktop-environment`] [`dropbox`] [`good-first-issue`] [`gui`] [`javascript`] [`nas`] [`open-source`] [`operating-system`] [`os`] [`osjs`] [`puter`] [`remote-desktop`] [`storage`] [`web-desktop`] [`web-os`] [`webtop`]  
...
- [qishibo/AnotherRedisDesktopManager](https://github.com/qishibo/AnotherRedisDesktopManager) ★33521 [`redis`] [`redis-client`] [`redis-cluster`] [`redis-desktop-manager`] [`redis-gui`]  
...
[查看更多 JavaScript 仓库 →](dist/2025120712/LANG_JavaScript.md)
...
- [gaoyifan/china-operator-ip](https://github.com/gaoyifan/china-operator-ip) ★3300 [`asn`] [`bgp`] [`china`] [`daily`] [`ip`] [`ipv6`] [`operator`]  
...
- [2dust/v2rayNG](https://github.com/2dust/v2rayNG) ★47909 [`android`] [`proxy`] [`shadowsocks`] [`socks5`] [`trojan`] [`v2fly`] [`v2ray`] [`vless`] [`vmess`] [`vpn`] [`xray`] [`xtls`]  
...
- [bannedbook/fanqiang](https://github.com/bannedbook/fanqiang) ★41045 [`brook`] [`daze`] [`fanqiang`] [`goflyway`] [`lightsocks`] [`proxy`] [`psiphon`] [`shadowsocks`] [`ssr`] [`trojan`] [`v2ray`]  
  这是一个用于科学上网的翻墙工具，提供多种连接方式，支持多种操作系统。它使用Go语言开发，核心模块包括代理服务器和连接管理，并整合了多种开源库。该项目旨在为用户提供快速、稳定、便捷的网络访问方式，绕过地区限制。
- [gkd-kit/gkd](https://github.com/gkd-kit/gkd) ★34038 [`accessibility`] [`android`] [`auto`] [`click`] [`compose`] [`jetpack-compose`] [`kotlin`] [`kotlin-js`] [`kotlin-multiplatform`] [`webassembly`]  
...
- [MetaCubeX/ClashMetaForAndroid](https://github.com/MetaCubeX/ClashMetaForAndroid) ★30680  
...
- [pppscn/SmsForwarder](https://github.com/pppscn/SmsForwarder) ★23652 [`android`] [`api`] [`app`] [`bark`] [`call`] [`chatgpt`] [`dingding`] [`forward`] [`mqtt`] [`pushdear`] [`pushplus`] [`serverchan`] [`sms`] [`smtp`] [`telegram`] [`webhook`] [`wechatapp`]  
...
- [RikkaApps/Shizuku](https://github.com/RikkaApps/Shizuku) ★19362  
...
- [MatsuriDayo/NekoBoxForAndroid](https://github.com/MatsuriDayo/NekoBoxForAndroid) ★17467 [`android`] [`gfw`] [`proxy`] [`shadowsocks`] [`sing-box`] [`trojan`] [`vmess`] [`vpn`]  
...
- [mihonapp/mihon](https://github.com/mihonapp/mihon) ★17330 [`android`] [`kotlin`] [`manga`] [`manga-downloader`] [`manga-reader`] [`mangadownloader`] [`mangareader`] [`tachiyomi`] [`tachiyomi-alternative`]  
...
- [tiann/KernelSU](https://github.com/tiann/KernelSU) ★14335 [`android`] [`kernel`] [`kernelsu`] [`root`] [`su`]  
...
[查看更多 Kotlin 仓库 →](dist/2025120712/LANG_Kotlin.md)
...
- [iDvel/rime-ice](https://github.com/iDvel/rime-ice) ★14288 [`rime`] [`rime-config`] [`rime-custom`] [`rime-schema`] [`rime-squirrel`] [`rime-weasel`]  
...
- [zhihulite/Hydrogen](https://github.com/zhihulite/Hydrogen) ★1347  
...
- [jaywcjlove/linux-command](https://github.com/jaywcjlove/linux-command) ★35170 [`bash`] [`chrome`] [`chrome-extension`] [`command-line`] [`gh-pages`] [`linux`] [`linux-command`] [`ls`] [`screen`] [`screenshot`] [`search`] [`ssh`] [`tools`] [`web-tools`]  
...
- [twbs/bootstrap](https://github.com/twbs/bootstrap) ★173773 [`bootstrap`] [`css`] [`css-framework`] [`html`] [`javascript`] [`sass`] [`scss`]  
...
- [996icu/996.ICU](https://github.com/996icu/996.ICU) ★274973  
  该项目是一个关于中国程序员工作强度和压力的匿名吐槽平台。用户可以通过匿名发表帖子，分享工作经历和感受。平台使用简单的文本格式，并支持搜索和过滤功能。该项目主要使用PHP语言开发，并使用数据库存储用户数据和帖子信息。
- [awesome-selfhosted/awesome-selfhosted](https://github.com/awesome-selfhosted/awesome-selfhosted) ★261819 [`awesome`] [`awesome-list`] [`cloud`] [`free-software`] [`hosting`] [`privacy`] [`self-hosted`] [`selfhosted`]  
  这是一个收集各种可自托管软件的资源列表。涵盖了从博客和论坛到视频会议和VPN等各种应用。列表按类别组织，便于用户快速找到所需的自托管工具。该项目使用纯文本格式维护，便于轻松更新和浏览。
- [justjavac/free-programming-books-zh_CN](https://github.com/justjavac/free-programming-books-zh_CN) ★115824 [`android`] [`angular`] [`books`] [`free`] [`ios`] [`javascript`] [`kotlin`] [`pdf`] [`programming`] [`python`] [`react`] [`react-native`] [`swift`] [`vue`]  
...
- [deepseek-ai/DeepSeek-R1](https://github.com/deepseek-ai/DeepSeek-R1) ★91555  
...
- [ruanyf/weekly](https://github.com/ruanyf/weekly) ★80229  
...
- [PlexPt/awesome-chatgpt-prompts-zh](https://github.com/PlexPt/awesome-chatgpt-prompts-zh) ★57081 [`chat-gpt`] [`chatgpt`] [`chatgpt3`] [`chatgpt4`] [`gpt`]  
...
- [justjavac/awesome-wechat-weapp](https://github.com/justjavac/awesome-wechat-weapp) ★49944 [`demo`] [`minapp`] [`miniprogram`] [`weapp-demo`] [`wechat`] [`wechat-weapp`] [`wepy`] [`wxapp`] [`wxml`]  
...
- [LC044/WeChatMsg](https://github.com/LC044/WeChatMsg) ★40195 [`chatgpt`] [`llms`] [`pyqt`] [`wechat`]  
...
[查看更多 NaN 仓库 →](dist/2025120712/LANG_NaN.md)
...
- [opa334/TrollStore](https://github.com/opa334/TrollStore) ★20512  
...
[查看更多 Objective-C 仓库 →](dist/2025120712/LANG_Objective-C.md)
...
- [cheat-engine/cheat-engine](https://github.com/cheat-engine/cheat-engine) ★17391  
...
- [stascorp/rdpwrap](https://github.com/stascorp/rdpwrap) ★16094  
...
- [easychen/howto-make-more-money](https://github.com/easychen/howto-make-more-money) ★18192  
...
[查看更多 PHP 仓库 →](dist/2025120712/LANG_PHP.md)
...
- [cmderdev/cmder](https://github.com/cmderdev/cmder) ★26679 [`c-plus-plus`] [`cmder`] [`color-scheme`] [`command-line`] [`conemu`] [`mintty`] [`portable`] [`powershell`] [`shell`] [`ssh-agent`] [`windows`]  
...
- [ScoopInstaller/Scoop](https://github.com/ScoopInstaller/Scoop) ★23258 [`installer`] [`powershell`] [`scoop`] [`windows`]  
...
- [public-apis/public-apis](https://github.com/public-apis/public-apis) ★382914 [`api`] [`apis`] [`dataset`] [`development`] [`free`] [`list`] [`lists`] [`open-source`] [`public`] [`public-api`] [`public-apis`] [`resources`] [`software`]  
...
- [EbookFoundation/free-programming-books](https://github.com/EbookFoundation/free-programming-books) ★378388 [`books`] [`education`] [`hacktoberfest`] [`list`] [`resource`]  
...
- [ytdl-org/youtube-dl](https://github.com/ytdl-org/youtube-dl) ★139060  
...
- [521xueweihan/HelloGitHub](https://github.com/521xueweihan/HelloGitHub) ★136431 [`awesome`] [`github`] [`hellogithub`] [`python`]  
...
- [deepseek-ai/DeepSeek-V3](https://github.com/deepseek-ai/DeepSeek-V3) ★100595  
...
- [nvbn/thefuck](https://github.com/nvbn/thefuck) ★94826 [`python`] [`shell`]  
...
- [home-assistant/core](https://github.com/home-assistant/core) ★83011 [`asyncio`] [`hacktoberfest`] [`home-automation`] [`internet-of-things`] [`iot`] [`mqtt`] [`python`] [`raspberry-pi`]  
...
- [fighting41love/funNLP](https://github.com/fighting41love/funNLP) ★77678  
...
- [sherlock-project/sherlock](https://github.com/sherlock-project/sherlock) ★70744 [`cli`] [`cti`] [`cybersecurity`] [`forensics`] [`hacktoberfest`] [`information-gathering`] [`infosec`] [`linux`] [`osint`] [`pentesting`] [`python`] [`python3`] [`reconnaissance`] [`redteam`] [`sherlock`] [`tools`]  
...
- [xtekky/gpt4free](https://github.com/xtekky/gpt4free) ★65628 [`chatbot`] [`chatbots`] [`chatgpt`] [`chatgpt-4`] [`chatgpt-api`] [`chatgpt-free`] [`chatgpt4`] [`deepseek`] [`deepseek-api`] [`deepseek-r1`] [`gpt`] [`gpt-4`] [`gpt-4o`] [`gpt4`] [`gpt4-api`] [`language-model`] [`openai`] [`openai-api`] [`openai-chatgpt`] [`reverse-engineering`]  
  该项目提供一个基于GPT-4的免费API访问途径。用户无需付费即可调用GPT-4模型进行文本生成、翻译等任务。该项目使用Python和OpenAI API进行开发，核心功能包括API接口封装和请求管理，并提供简易的调用示例。

[查看更多 Python 仓库 →](dist/2025120712/LANG_Python.md)
...
- [TapXWorld/ChinaTextbook](https://github.com/TapXWorld/ChinaTextbook) ★61078  
...
- [pyenv/pyenv](https://github.com/pyenv/pyenv) ★43820 [`python`] [`shell`]  
...
- [discourse/discourse](https://github.com/discourse/discourse) ★45726 [`discourse`] [`ember`] [`forum`] [`javascript`] [`postgresql`] [`rails`] [`ruby`]  
...
- [rustdesk/rustdesk](https://github.com/rustdesk/rustdesk) ★103552 [`android`] [`anydesk`] [`dart`] [`flatpak`] [`flutter`] [`flutter-apps`] [`ios`] [`linux`] [`macos`] [`p2p`] [`rdp`] [`remote-control`] [`remote-desktop`] [`rust`] [`rust-lang`] [`teamviewer`] [`vnc`] [`wayland`] [`windows`]  
...
- [zed-industries/zed](https://github.com/zed-industries/zed) ★70972 [`gpui`] [`rust-lang`] [`text-editor`] [`zed`]  
...
- [topjohnwu/Magisk](https://github.com/topjohnwu/Magisk) ★57382  
...
- [starship/starship](https://github.com/starship/starship) ★52649 [`bash`] [`fish`] [`fish-prompt`] [`fish-theme`] [`oh-my-zsh`] [`powershell`] [`rust`] [`shell-prompt`] [`starship`] [`zsh`] [`zsh-prompt`] [`zsh-theme`]  
...
- [dani-garcia/vaultwarden](https://github.com/dani-garcia/vaultwarden) ★51658 [`bitwarden`] [`bitwarden-rs`] [`docker`] [`rocket`] [`rust`] [`vaultwarden`]  
...
- [tw93/Pake](https://github.com/tw93/Pake) ★43730 [`chatgpt`] [`claude`] [`desktop`] [`gemini`] [`hight-performance`] [`linux`] [`macos`] [`no-electron`] [`package`] [`rust`] [`tauri`] [`windows`] [`youtube`]  
...
- [TabbyML/tabby](https://github.com/TabbyML/tabby) ★32540 [`ai`] [`codegen`] [`coding-assistant`] [`coding-language`] [`developer-experience`] [`developer-tools`] [`gen-ai`] [`ide`] [`llms`]  
...
- [fish-shell/fish-shell](https://github.com/fish-shell/fish-shell) ★31774 [`fish`] [`rust`] [`shell`] [`terminal`]  
...
- [lsd-rs/lsd](https://github.com/lsd-rs/lsd) ★15168 [`color`] [`hacktoberfest`] [`icons`] [`ls`] [`nerd-fonts`] [`rust`]  
...
- [rathole-org/rathole](https://github.com/rathole-org/rathole) ★12424 [`firewall`] [`frp`] [`http`] [`nat`] [`network`] [`ngrok`] [`noise`] [`noise-protocol`] [`proxy`] [`rust`] [`tunnel`]  
...
[查看更多 Rust 仓库 →](dist/2025120712/LANG_Rust.md)
...
- [ohmyzsh/ohmyzsh](https://github.com/ohmyzsh/ohmyzsh) ★183167 [`cli`] [`cli-app`] [`oh-my-zsh`] [`oh-my-zsh-plugin`] [`oh-my-zsh-theme`] [`ohmyzsh`] [`plugin-framework`] [`plugins`] [`productivity`] [`shell`] [`terminal`] [`theme`] [`themes`] [`zsh`] [`zsh-configuration`]  
...
- [sickcodes/Docker-OSX](https://github.com/sickcodes/Docker-OSX) ★51813 [`container`] [`docker`] [`docker-osx`] [`kvm`] [`macos`] [`os`] [`osx`] [`osx-kvm`] [`x`] [`x11`]  
...
- [dockur/windows](https://github.com/dockur/windows) ★48960 [`docker`] [`docker-container`] [`virtualization`] [`windows`] [`windows-virtual-desktop`] [`windows-virtual-machine`] [`windows-virtual-machines`] [`windows-vm`]  
...
- [acmesh-official/acme.sh](https://github.com/acmesh-official/acme.sh) ★44796 [`acme`] [`acme-challenge`] [`acme-protocol`] [`acme-v2`] [`ash`] [`bash`] [`buypass`] [`certbot`] [`lets-encrypt`] [`posix`] [`posix-sh`] [`shell`] [`ssl`] [`ssl-certificate`] [`ssl-certificates`] [`tls`] [`tls-certificate`] [`zerossl`]  
...
- [zsh-users/zsh-autosuggestions](https://github.com/zsh-users/zsh-autosuggestions) ★34348 [`autocomplete`] [`autosuggest`] [`fish`] [`shell`] [`zsh`] [`zsh-autosuggestions`]  
...
- [233boy/v2ray](https://github.com/233boy/v2ray) ★27546 [`shell-script`] [`v2ray`] [`vmess`]  
  这是一个基于Go语言开发的V2Ray代理服务器。它提供快速、稳定的代理连接，支持多种协议和配置。项目包含了核心代理服务以及客户端配置文件生成工具，用户可以方便地配置和管理V2Ray连接。代码结构清晰，易于维护和扩展。
- [hwdsl2/setup-ipsec-vpn](https://github.com/hwdsl2/setup-ipsec-vpn) ★27080 [`cisco-ipsec`] [`docker`] [`encryption`] [`ikev2`] [`ipsec`] [`l2tp`] [`libreswan`] [`linux`] [`network`] [`raspberry-pi`] [`security`] [`vpn`] [`vpn-client`] [`vpn-server`]  
...
- [yuaotian/go-cursor-help](https://github.com/yuaotian/go-cursor-help) ★25178 [`cursor`] [`error`]  
...
- [lxgw/LxgwWenKai](https://github.com/lxgw/LxgwWenKai) ★22330 [`cjk`] [`font`] [`typeface`]  
...
[查看更多 Shell 仓库 →](dist/2025120712/LANG_Shell.md)

## Svelte

- [imputnet/cobalt](https://github.com/imputnet/cobalt) ★37483 [`collaboration`] [`downloader`] [`instagram`] [`javascript`] [`music`] [`reddit`] [`social-media`] [`soundcloud`] [`svelte`] [`tiktok`] [`twitter`] [`typescript`] [`video`] [`vimeo`] [`vk`] [`webapp`] [`youtube`] [`youtube-downloader`]  
...
- [GoodiesHQ/headscale-admin](https://github.com/GoodiesHQ/headscale-admin) ★953  
...
- [Finb/Bark](https://github.com/Finb/Bark) ★7132 [`ios`] [`ios-app`] [`ios-swift`] [`mvvm`] [`mvvm-architecture`] [`push-notifications`] [`rxcocoa`] [`rxdatasources`] [`rxswift`]  
...
- [X1a0He/Adobe-Downloader](https://github.com/X1a0He/Adobe-Downloader) ★2592  
...
- [iptv-org/iptv](https://github.com/iptv-org/iptv) ★104663 [`iptv`] [`m3u`] [`playlist`] [`streams`] [`tv`]  
...
- [ant-design/ant-design](https://github.com/ant-design/ant-design) ★96883 [`ant-design`] [`antd`] [`design-systems`] [`react`] [`typescript`] [`ui-kit`] [`ui-library`]  
...
- [tailwindlabs/tailwindcss](https://github.com/tailwindlabs/tailwindcss) ★91400 [`css`] [`css-framework`] [`functional-css`] [`postcss`] [`responsive`] [`tailwindcss`] [`utility-classes`]  
...
- [ChatGPTNextWeb/NextChat](https://github.com/ChatGPTNextWeb/NextChat) ★86614 [`calclaude`] [`chatgpt`] [`claude`] [`cross-platform`] [`desktop`] [`fe`] [`gemini`] [`gemini-pro`] [`gemini-server`] [`gemini-ultra`] [`gpt-4o`] [`groq`] [`nextjs`] [`ollama`] [`react`] [`tauri`] [`tauri-app`] [`vercel`] [`webui`]  
...
- [google-gemini/gemini-cli](https://github.com/google-gemini/gemini-cli) ★86105 [`gemini`] [`gemini-api`]  
...
- [clash-verge-rev/clash-verge-rev](https://github.com/clash-verge-rev/clash-verge-rev) ★85856 [`clash`] [`clash-meta`] [`clash-verge`] [`linux`] [`mac`] [`mihomo`] [`tauri-app`] [`windows`]  
...
- [immich-app/immich](https://github.com/immich-app/immich) ★85849 [`backup-tool`] [`flutter`] [`google-photos`] [`google-photos-alternative`] [`javascript`] [`mobile-app`] [`nestjs`] [`nodejs`] [`photo-gallery`] [`photos`] [`photos-management`] [`self-hosted`] [`svelte`] [`sveltekit`] [`typescript`] [`videos`]  
...
- [mermaid-js/mermaid](https://github.com/mermaid-js/mermaid) ★84551 [`diagrams`] [`diagrams-as-code`] [`documentation`] [`flowchart`] [`javascript`] [`mindmap`] [`typescript`] [`uml-diagrams`]  
...
- [Stirling-Tools/Stirling-PDF](https://github.com/Stirling-Tools/Stirling-PDF) ★70817 [`docker`] [`hacktoberfest`] [`java`] [`pdf`] [`pdf-converter`] [`pdf-editor`] [`pdf-manipulation`] [`pdf-merger`] [`pdf-ocr`] [`pdf-tools`] [`pdf-web-apps`] [`pdfmerger`]  
...
[查看更多 TypeScript 仓库 →](dist/2025120712/LANG_TypeScript.md)
...
- [CorentinTh/it-tools](https://github.com/CorentinTh/it-tools) ★34945 [`converter`] [`developer-productivity`] [`developer-tools`] [`frontend`] [`javascript`] [`productivity`] [`tool`] [`tools`] [`typescript`] [`vuejs`] [`website`]  
...
- [vbenjs/vue-vben-admin](https://github.com/vbenjs/vue-vben-admin) ★30807 [`admin-template`] [`ant-design`] [`element-plus`] [`monorepo`] [`naive-ui`] [`shadcn-ui`] [`tsx`] [`typescript`] [`vben`] [`vite`] [`vite-template`] [`vue-router`] [`vue3`] [`vuex`]  
...
- [requarks/wiki](https://github.com/requarks/wiki) ★27432 [`cross-platform`] [`documentation`] [`git`] [`markdown`] [`nodejs`] [`open-source`] [`wiki`]  
...
- [Lissy93/dashy](https://github.com/Lissy93/dashy) ★23291 [`awesome`] [`dashboard`] [`docker`] [`hacktoberfest`] [`homelab`] [`homepage`] [`mit`] [`nodejs`] [`organization`] [`productivity`] [`pwa`] [`self-hosted`] [`startpage`] [`vue`]  
...
- [zxwk1998/vue-admin-better](https://github.com/zxwk1998/vue-admin-better) ★18343 [`admin`] [`admin-dashboard`] [`admin-template`] [`ant-design`] [`ant-design-vue`] [`axios`] [`element`] [`element-plus`] [`element-ui`] [`spring-boot`] [`vite`] [`vue`] [`vue-admin`] [`vue-cli`] [`vue-element-admin`] [`vue-template`] [`vue3`] [`vue3-admin`] [`vue3-composition-api`] [`vuejs`]  
...
[查看更多 Vue 仓库 →](dist/2025120712/LANG_Vue.md)
