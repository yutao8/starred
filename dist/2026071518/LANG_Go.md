...
- [QuantumNous/new-api](https://github.com/QuantumNous/new-api) ★42296 [`ai-gateway`] [`claude`] [`deepseek`] [`gemini`] [`newapi`] [`openai`] [`rerank`]  
...
- [juanfont/headscale](https://github.com/juanfont/headscale) ★41755 [`tailscale`] [`tailscale-control-server`] [`tailscale-server`] [`wireguard`]  
...
- [schollz/croc](https://github.com/schollz/croc) ★35554 [`data-transfer`] [`file-sharing`] [`golang`] [`pake`] [`peer-to-peer`] [`tcp`] [`transfer`]  
  croc 是一个命令行工具，用于在两台计算机之间轻松、安全地传输文件。它通过一个中继服务器建立直接连接，支持端到端加密，允许用户仅通过简单的口令或生成的代码实现跨平台文件共享，适用于快速发送文件而无需配置复杂的网络环境。
- [filebrowser/filebrowser](https://github.com/filebrowser/filebrowser) ★35551 [`file-browser`] [`file-manager`] [`file-sharing`] [`go`] [`material-design`] [`self-hosted`] [`vue`]  
...
- [tailscale/tailscale](https://github.com/tailscale/tailscale) ★33989 [`2fa`] [`oauth`] [`sso`] [`tailscale`] [`vpn`] [`wireguard`]  
...
- [binwiederhier/ntfy](https://github.com/binwiederhier/ntfy) ★31871 [`curl`] [`notifications`] [`ntfy`] [`ntfysh`] [`pubsub`] [`push-notifications`] [`rest-api`]  
...
- [GopeedLab/gopeed](https://github.com/GopeedLab/gopeed) ★25240 [`android`] [`bittorrent`] [`cross-platform`] [`debian`] [`downloader`] [`flutter`] [`golang`] [`http`] [`https`] [`ios`] [`macos`] [`magnet`] [`torrent`] [`ubuntu`] [`windows`]  
...
- [OpenListTeam/OpenList](https://github.com/OpenListTeam/OpenList) ★23543 [`alist`] [`aliyunpan`] [`baidupan`] [`openlist`]  
  一个开源的列表协作工具，支持多人实时编辑和数据同步，可用于任务管理、资源整理等场景，提供简洁的用户界面和灵活的数据组织方式，便于团队高效协作与信息共享。
- [navidrome/navidrome](https://github.com/navidrome/navidrome) ★22309 [`airsonic`] [`madsonic`] [`media-server`] [`music`] [`music-server`] [`navidrome`] [`opensubsonic`] [`raspberry-pi`] [`self-hosted`] [`streamer`] [`streaming-api`] [`streaming-audio`] [`subsonic`] [`subsonic-server`]  
...
- [apernet/hysteria](https://github.com/apernet/hysteria) ★22105 [`censorship-circumvention`] [`golang`] [`http-proxy`] [`hysteria`] [`proxy`] [`quic`] [`relay`] [`reliable-udp`] [`socks5`] [`tun`] [`vpn`]  
  Apernet Hysteria 是一个基于 QUIC 协议的高性能网络穿透工具，主要用于优化弱网环境下的数据传输，支持多路并发、低延迟传输，适用于跨境网络加速、远程访问等场景，具有良好的抗丢包和拥塞控制能力。
- [chaitin/SafeLine](https://github.com/chaitin/SafeLine) ★21794 [`api-gateway`] [`application-security`] [`appsec`] [`blueteam`] [`bruteforce`] [`captcha`] [`cve`] [`cybersecurity`] [`firewall`] [`hackers`] [`http-flood`] [`security`] [`self-hosted`] [`sql-injection`] [`vulnerability`] [`waf`] [`web-application-firewall`] [`web-security`] [`websecurity`] [`xss`]  
...
- [putyy/res-downloader](https://github.com/putyy/res-downloader) ★18626 [`douyin`] [`kuaishou`] [`res-downloader`] [`wechat`] [`wechat-video`] [`xiaohongshu`]  
  用于下载网页资源的工具，支持从指定URL抓取文件、图片、视频等资源并保存到本地，通常用于批量下载或离线备份，适合需要提取网页静态资源的场景。
- [ginuerzh/gost](https://github.com/ginuerzh/gost) ★18099 [`dns`] [`go`] [`golang`] [`http2`] [`kcp`] [`obfs4`] [`quic`] [`shadowsocks`] [`sni`] [`socks5`] [`ssh`] [`tls`] [`tunnel`] [`tuntap`] [`udp`]  
...
- [ory/hydra](https://github.com/ory/hydra) ★17397 [`authorization`] [`cloud`] [`docker`] [`federation`] [`hacktoberfest`] [`hydra`] [`identity`] [`oauth`] [`oauth-provider`] [`oauth2`] [`oauth2-provider`] [`oauth2-server`] [`oidc`] [`openid`] [`openid-connect`] [`openid-connect-provider`] [`openid-provider`] [`security`] [`server`] [`sso`]  
...
- [gotify/server](https://github.com/gotify/server) ★15297 [`api`] [`cloud`] [`free-software`] [`golang`] [`gotify`] [`hosting`] [`javascript`] [`notifications`] [`privacy`] [`react`] [`self-hosted`] [`self-hosting`] [`selfhosted`]  
...
- [cloudflare/cloudflared](https://github.com/cloudflare/cloudflared) ★14858 [`cloudflare`] [`cloudflare-tunnel`] [`reverse-proxy`] [`zero-trust-network-access`]  
...
- [shadow1ng/fscan](https://github.com/shadow1ng/fscan) ★14145 [`fscan`] [`scanner`] [`scanning`] [`sec`] [`security`] [`security-tools`]  
  一款内网安全扫描工具，主要用于自动化检测常见服务的弱口令、漏洞和安全风险，支持多种协议和自定义脚本扩展，适合渗透测试和安全评估场景。
- [casdoor/casdoor](https://github.com/casdoor/casdoor) ★13943 [`agent`] [`agentic-ai`] [`agi`] [`ai-gateway`] [`auth`] [`authentication`] [`iam`] [`llm`] [`mcp`] [`mcp-gateway`] [`mfa`] [`oauth`] [`oidc`] [`openclaw`] [`radius`] [`saml`] [`scim`] [`single-sign-on`] [`sso`] [`webauthn`]  
...
- [fullstorydev/grpcurl](https://github.com/fullstorydev/grpcurl) ★12735 [`golang`] [`grpc`] [`protobuf`]  
...
- [nxtrace/NTrace-core](https://github.com/nxtrace/NTrace-core) ★8021 [`api`] [`as-path`] [`asn-lookup`] [`geoip`] [`geolocation`] [`ip-lookup`] [`network-analysis`] [`next-trace`] [`nexttrace`] [`traceroute`] [`utility`]  
...
- [qjfoidnh/BaiduPCS-Go](https://github.com/qjfoidnh/BaiduPCS-Go) ★5302 [`baidupcs`]  
...
- [samwafgo/SamWaf](https://github.com/samwafgo/SamWaf) ★1495  
...
- [songloft-org/songloft](https://github.com/songloft-org/songloft) ★1087 [`docker`] [`docker-compose`] [`home-server`] [`miot`] [`music`] [`music-player`] [`music-server`] [`nas`] [`navidrome`] [`songloft`] [`subsonic`] [`webdav`] [`xiaomusic`]  
