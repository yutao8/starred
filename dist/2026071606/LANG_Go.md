...
- [fatedier/frp](https://github.com/fatedier/frp) ★108066 [`expose`] [`firewall`] [`frp`] [`go`] [`http-proxy`] [`nat`] [`p2p`] [`proxy`] [`reverse-proxy`] [`tunnel`]  
  frp 是一个快速反向代理工具，主要用于将内网服务通过具有公网 IP 的节点进行穿透，支持 TCP、UDP、HTTP、HTTPS 等多种协议，常用于在没有公网 IP 的环境下对外暴露本地服务，适合远程访问、调试和搭建私有服务。
- [syncthing/syncthing](https://github.com/syncthing/syncthing) ★86405 [`go`] [`p2p`] [`peer-to-peer`] [`synchronization`]  
  SyncThing 是一个开源的分布式文件同步工具，用于在多台设备间安全地共享和同步文件。它不依赖中心服务器，所有数据直接在设备间加密传输，确保隐私与安全。用户可通过本地网络或互联网实现跨平台文件实时同步，适用于备份、协作和文件管理场景。
- [junegunn/fzf](https://github.com/junegunn/fzf) ★81751 [`bash`] [`cli`] [`fish`] [`fzf`] [`go`] [`neovim`] [`tmux`] [`unix`] [`vim`] [`zsh`]  
  一个用 Go 编写的命令行模糊查找工具，支持实时搜索、精确匹配和扩展正则表达式，主要用于快速筛选文件、命令历史、进程列表等文本输入，提供交互式界面并可集成到 shell、编辑器中提升操作效率。
- [usememos/memos](https://github.com/usememos/memos) ★61563 [`docker`] [`foss`] [`go`] [`markdown`] [`memo`] [`microblog`] [`note-taking`] [`notecard`] [`own-your-data`] [`react`] [`self-hosted`] [`social-network`] [`sqlite`]  
  一个开源的、轻量级的笔记与知识管理工具，支持 Markdown 编辑、标签分类和实时同步，主要用于个人知识记录、日常想法整理和技术笔记存储，适合追求简洁高效写作体验的用户。
- [minio/minio](https://github.com/minio/minio) ★61341 [`amazon-s3`] [`cloud`] [`cloudnative`] [`cloudstorage`] [`go`] [`k8s`] [`kubernetes`] [`multi-cloud`] [`multi-cloud-kubernetes`] [`objectstorage`] [`s3`] [`storage`]  
...
- [FiloSottile/mkcert](https://github.com/FiloSottile/mkcert) ★59302 [`certificates`] [`chrome`] [`firefox`] [`https`] [`ios`] [`linux`] [`local-development`] [`localhost`] [`macos`] [`root-ca`] [`tls`] [`windows`]  
...
- [rclone/rclone](https://github.com/rclone/rclone) ★58347 [`azure-blob`] [`azure-blob-storage`] [`azure-files`] [`backblaze-b2`] [`cloud-storage`] [`dropbox`] [`encryption`] [`ftp`] [`fuse-filesystem`] [`go`] [`golang`] [`google-cloud-storage`] [`google-drive`] [`onedrive`] [`openstack-swift`] [`rclone`] [`s3`] [`sftp`] [`sync`] [`webdav`]  
  rclone 是一个命令行工具，用于同步、上传和下载文件到多种云存储服务，支持包括 Amazon S3、Google Drive、Dropbox 等在内的数十种云存储平台，常用于备份和文件迁移场景。
- [go-gitea/gitea](https://github.com/go-gitea/gitea) ★56838 [`bitbucket`] [`cicd`] [`devops`] [`docker-registry-v2`] [`git`] [`git-gui`] [`git-lfs`] [`git-server`] [`gitea`] [`github`] [`github-actions`] [`gitlab`] [`go`] [`golang`] [`hacktoberfest`] [`maven-server`] [`npm-registry`] [`self-hosted`] [`typescript`] [`vue`]  
...
- [AlistGo/alist](https://github.com/AlistGo/alist) ★49873 [`file-server`] [`gin`] [`golang`] [`onedrive`] [`solidjs`] [`webdav`]  
...
- [coreybutler/nvm-windows](https://github.com/coreybutler/nvm-windows) ★47064 [`go`] [`management`] [`node`] [`node-version-manager`] [`nodejs`] [`nvm`] [`switch`] [`version`] [`version-manager`] [`versioning`] [`windows`]  
...
- [QuantumNous/new-api](https://github.com/QuantumNous/new-api) ★42333 [`ai-gateway`] [`claude`] [`deepseek`] [`gemini`] [`newapi`] [`openai`] [`rerank`]  
  该项目主要用于构建新一代API服务，聚焦于提升系统间通信效率与数据处理能力，通常适用于需要高并发、低延迟场景的分布式系统集成，帮助开发者简化后端服务对接与业务逻辑封装。
- [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) ★42175 [`antigravity`] [`claude-code`] [`cluade`] [`codex`] [`gemini`] [`openai`]  
  CLIProxyAPI 是一个用于简化命令行工具与 API 服务之间交互的代理接口，主要用于将 CLI 请求转发为 HTTP 调用，支持鉴权、参数映射和响应格式化，适合集成第三方服务或构建自动化工作流。
- [juanfont/headscale](https://github.com/juanfont/headscale) ★41780 [`tailscale`] [`tailscale-control-server`] [`tailscale-server`] [`wireguard`]  
...
- [schollz/croc](https://github.com/schollz/croc) ★35563 [`data-transfer`] [`file-sharing`] [`golang`] [`pake`] [`peer-to-peer`] [`tcp`] [`transfer`]  
  croc 是一个命令行工具，用于在两台计算机之间轻松、安全地传输文件。它通过一个中继服务器建立直接连接，支持端到端加密，允许用户仅通过简单的口令或生成的代码实现跨平台文件共享，适用于快速发送文件而无需配置复杂的网络环境。
- [filebrowser/filebrowser](https://github.com/filebrowser/filebrowser) ★35555 [`file-browser`] [`file-manager`] [`file-sharing`] [`go`] [`material-design`] [`self-hosted`] [`vue`]  
...
- [tailscale/tailscale](https://github.com/tailscale/tailscale) ★34020 [`2fa`] [`oauth`] [`sso`] [`tailscale`] [`vpn`] [`wireguard`]  
  Tailscale 用于创建安全的私有网络，让设备和服务器通过加密连接轻松互通，无需手动配置防火墙或公网IP，适用于远程访问、服务发现和跨云网络互联，基于 WireGuard 实现，支持多种平台和自动化运维。
- [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api) ★32319 [`2api`] [`antigravity2api`] [`cc2api`] [`claude`] [`claude-code`] [`codex`] [`crs`] [`crs2`] [`gemini`]  
  sub2api 是一个将订阅链接转换为 API 接口的工具，主要用于将网络订阅服务中的资源列表转换为可通过 HTTP 请求访问的 API 接口，方便用户在自定义应用或脚本中动态获取和管理订阅内容，适用于需要自动化处理订阅源的场景。
- [binwiederhier/ntfy](https://github.com/binwiederhier/ntfy) ★31894 [`curl`] [`notifications`] [`ntfy`] [`ntfysh`] [`pubsub`] [`push-notifications`] [`rest-api`]  
...
- [cloudreve/cloudreve](https://github.com/cloudreve/cloudreve) ★28307 [`cloud`] [`cloud-storage`] [`cloudreve`] [`file`] [`file-manager`] [`file-sharing`] [`golang`]  
  Cloudreve 是一个支持多存储的私有云存储系统，可用于快速搭建个人或企业网盘服务，支持本地、阿里云OSS、腾讯云COS、OneDrive等存储后端，提供文件管理、分享、上传下载、权限控制和用户管理等功能，适合用于文件托管与协作。
- [XIU2/CloudflareSpeedTest](https://github.com/XIU2/CloudflareSpeedTest) ★27926 [`cdn`] [`cloudflare`] [`go`] [`golang`] [`speedtest`]  
  用于测试 Cloudflare CDN 节点的延迟和下载速度，支持自动优选域名、IP 扫描与速度测试，帮助用户快速发现低延迟、高速率的 Cloudflare 节点，常用于搭建代理或加速服务时的节点筛选。
- [Wox-launcher/Wox](https://github.com/Wox-launcher/Wox) ★27144 [`alfred`] [`launcher`] [`launchy`] [`productivity`] [`productivity-tools`] [`raycast`] [`spotlight`] [`wox`] [`wox-launcher`]  
  Wox 是一个基于 Electron 的开源桌面启动器，主要用于快速搜索和启动应用程序、文件、文件夹及执行自定义命令，支持插件扩展与主题定制，适用于 Windows 平台，提升用户操作效率。
- [GopeedLab/gopeed](https://github.com/GopeedLab/gopeed) ★25248 [`android`] [`bittorrent`] [`cross-platform`] [`debian`] [`downloader`] [`flutter`] [`golang`] [`http`] [`https`] [`ios`] [`macos`] [`magnet`] [`torrent`] [`ubuntu`] [`windows`]  
...
- [OpenListTeam/OpenList](https://github.com/OpenListTeam/OpenList) ★23552 [`alist`] [`aliyunpan`] [`baidupan`] [`openlist`]  
  一个开源的列表协作工具，支持多人实时编辑和数据同步，可用于任务管理、资源整理等场景，提供简洁的用户界面和灵活的数据组织方式，便于团队高效协作与信息共享。
- [navidrome/navidrome](https://github.com/navidrome/navidrome) ★22320 [`airsonic`] [`madsonic`] [`media-server`] [`music`] [`music-server`] [`navidrome`] [`opensubsonic`] [`raspberry-pi`] [`self-hosted`] [`streamer`] [`streaming-api`] [`streaming-audio`] [`subsonic`] [`subsonic-server`]  
...
- [apernet/hysteria](https://github.com/apernet/hysteria) ★22108 [`censorship-circumvention`] [`golang`] [`http-proxy`] [`hysteria`] [`proxy`] [`quic`] [`relay`] [`reliable-udp`] [`socks5`] [`tun`] [`vpn`]  
  Apernet Hysteria 是一个基于 QUIC 协议的高性能网络穿透工具，主要用于优化弱网环境下的数据传输，支持多路并发、低延迟传输，适用于跨境网络加速、远程访问等场景，具有良好的抗丢包和拥塞控制能力。
- [chaitin/SafeLine](https://github.com/chaitin/SafeLine) ★21808 [`api-gateway`] [`application-security`] [`appsec`] [`blueteam`] [`bruteforce`] [`captcha`] [`cve`] [`cybersecurity`] [`firewall`] [`hackers`] [`http-flood`] [`security`] [`self-hosted`] [`sql-injection`] [`vulnerability`] [`waf`] [`web-application-firewall`] [`web-security`] [`websecurity`] [`xss`]  
...
- [putyy/res-downloader](https://github.com/putyy/res-downloader) ★18632 [`douyin`] [`kuaishou`] [`res-downloader`] [`wechat`] [`wechat-video`] [`xiaohongshu`]  
...
- [slackhq/nebula](https://github.com/slackhq/nebula) ★17528  
...
- [jeessy2/ddns-go](https://github.com/jeessy2/ddns-go) ★17064 [`alidns`] [`baiduyun`] [`cloudflare`] [`ddns`] [`ddns-go`] [`dnspod`] [`porkbun`]  
  ddns-go 是一个基于 Go 语言开发的轻量级动态域名解析（DDNS）工具，主要用于将动态公网 IP 地址自动更新到域名解析记录中，支持主流云服务商的 DNS API，适合自建服务器、家庭宽带等场景实现域名动态解析。
- [snail007/goproxy](https://github.com/snail007/goproxy) ★17006 [`dns-proxy`] [`encryption-proxy`] [`http`] [`http-proxy`] [`https-proxy`] [`kcptun`] [`reverse-proxy`] [`socket`] [`socks5`] [`socks5-proxy`] [`ss-proxy`] [`tcp`] [`tcp-proxy`] [`tls`] [`tls-proxy`] [`transparent-proxy`] [`udp`] [`udp-proxy`] [`upstream-proxies`] [`websocket-proxy`]  
  snail007/goproxy 是一个用 Go 编写的高性能内网穿透代理工具，支持 TCP、UDP、HTTP、HTTPS 等多种协议，主要用于将内网服务暴露到公网，适合远程访问、服务调试等场景，具备跨平台、配置简单、加密传输等特点。
- [gotify/server](https://github.com/gotify/server) ★15321 [`api`] [`cloud`] [`free-software`] [`golang`] [`gotify`] [`hosting`] [`javascript`] [`notifications`] [`privacy`] [`react`] [`self-hosted`] [`self-hosting`] [`selfhosted`]  
...
- [cloudflare/cloudflared](https://github.com/cloudflare/cloudflared) ★14865 [`cloudflare`] [`cloudflare-tunnel`] [`reverse-proxy`] [`zero-trust-network-access`]  
...
- [casdoor/casdoor](https://github.com/casdoor/casdoor) ★13949 [`agent`] [`agentic-ai`] [`agi`] [`ai-gateway`] [`auth`] [`authentication`] [`iam`] [`llm`] [`mcp`] [`mcp-gateway`] [`mfa`] [`oauth`] [`oidc`] [`openclaw`] [`radius`] [`saml`] [`scim`] [`single-sign-on`] [`sso`] [`webauthn`]  
...
- [0xJacky/nginx-ui](https://github.com/0xJacky/nginx-ui) ★11311 [`code-completion`] [`copilot`] [`cron`] [`docker`] [`go`] [`letsencrypt`] [`linux`] [`macos`] [`mcp`] [`mcp-server`] [`nginx`] [`self-hosted`] [`vue`] [`webui`] [`windows`]  
...
- [certimate-go/certimate](https://github.com/certimate-go/certimate) ★8798 [`acme`] [`acme-client`] [`automation`] [`certbot`] [`certificate`] [`certificate-management`] [`certificate-manager`] [`certificates`] [`devops`] [`google-trust-services`] [`https`] [`https-certificate`] [`lego`] [`letsencrypt`] [`self-hosted`] [`ssl`] [`ssl-cert`] [`ssl-certificate`] [`ssl-certificates`] [`zerossl`]  
...
- [iyear/tdl](https://github.com/iyear/tdl) ★7791 [`bash`] [`download`] [`downloader`] [`telegram`] [`telegram-api`] [`telegram-bot`] [`telegram-bots`] [`telegram-channel`] [`telegram-userbot`]  
...
- [Ackites/KillWxapkg](https://github.com/Ackites/KillWxapkg) ★5900  
...
- [songloft-org/songloft](https://github.com/songloft-org/songloft) ★1092 [`docker`] [`docker-compose`] [`home-server`] [`miot`] [`music`] [`music-player`] [`music-server`] [`nas`] [`navidrome`] [`songloft`] [`subsonic`] [`webdav`] [`xiaomusic`]  
